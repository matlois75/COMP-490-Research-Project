...
def main():
    ...
    f_optimizer = torch.optim.RMSprop(forward_params, lr=float(HYPERPARAMS["forward_lr"]), weight_decay=float(HYPERPARAMS["forward_lr_decay"]))
    g_optimizer = torch.optim.RMSprop(inverse_params, lr=float(HYPERPARAMS["inverse_lr"]))
    
    f_sched = torch.optim.lr_scheduler.CosineAnnealingLR(f_optimizer, HYPERPARAMS["epochs"])
    g_sched = torch.optim.lr_scheduler.CosineAnnealingLR(g_optimizer, HYPERPARAMS["epochs"])
